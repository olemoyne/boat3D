...
//			Decimal sub = v.getDecY().multiply(vplusun.getDecZ()).minus(v.getDecZ().multiply(vplusun.getDecY()));
	...
	/**
	 * Calcule l'intersection entre une surface et un segment
	 * @param interieur
	 * @param s
	 * @return
	 */
	public static Segment getIntersection(Area area, Segment s) {
		Vecteur v1 = null;
		Vecteur v2 = null;
		
		for (Segment l : area.getSegments()) {
			Segment v = l.intersection(s);
			if (v != null) {
				if (v1 == null) {
					v1 = v.getA();
					v2 = v.getB();
	...
	public static Decimal getSurface(Area surf, int axis) {
		if (surf == null) return Decimal.ZERO;
		Polygon2D pol = CalculSurface.getPoly(surf.points, axis);
		Decimal vol = new Decimal(pol.area());
		return vol;
	}

	public static Vecteur getCentre(Area surface, int axis) {
		Polygon2D poly = CalculSurface.getPoly(surface.points, axis);
		Vecteur v =getCentre (poly, surface.points.get(0).getDec(axis), axis);
		return v;
	}
