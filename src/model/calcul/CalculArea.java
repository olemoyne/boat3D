...
**/		
    ...
    public static Area reduceNormal(Area a, Decimal ep, int axis) {
        ...
        for (int i = 0; i < t.size(); i++) {
            ...
            if (angle.isPositive() == sens)  {
            	...
            	if (pt.getDecX().isPositive()) {
                    if (r1.getDecX().isPositive()) pts.add(r1); 
                    if (r2.getDecX().isPositive()) pts.add(r2); 
            	} else {
                    if (!r1.getDecX().isPositive()) pts.add(r1); 
                    if (!r2.getDecX().isPositive()) pts.add(r2); 
            	}
                ...
                if (inter == null) {
                	...
                	if (pt.getDecX().isPositive()) {
                        if (inter.getDecX().isPositive()) pts.add(inter); 
                	} else {
                        if (!inter.getDecX().isPositive()) pts.add(inter); 
                	}
                }
	...
	public static Area truncate(Area proj, Plan3D pl) {
		if (proj == null) return null;
		if (pl == null) return null;
		
		ArrayList<Vecteur> points = new ArrayList<Vecteur> (); 
		if (proj.size() == 0) return new Area(points);
		...
		for (Vecteur pt : proj.points) {
			if (last == null) {
				...
				if (pl.donneCote(pt) > 0) {
					if (!inside) { /// Ajouter l'intersection
						Vecteur inter = pl.intersection(last, pt);
						if (inter != null) {
							points.add(inter);
							last = inter;
						}
	...
	/**
	 *  Intersection entre une surface et un plan 
	 * 
	 * @param contours
	 * @param pl
	 * @param xaxis
	 * @return
	 */
	public static Area intersection(Area aire, Plan3D pl) {
		Area ret = new Area();
		for (Segment seg : aire.getSegments()) {
			Vecteur inter = pl.intersection(seg.getA(), seg.getB());
			if (inter != null) ret.points.add(inter);
		}
		return ret;
	}
