...
public class MapDeVecteurs implements Serializable {
	...
	/** 
	 * Construction d'une forme de type Area avec l'intersection de la map avec le plan
	 * 
	 * @param pl
	 * @return
	 */
	 ...
	 * @return
	 */
	public Area intersectionHorizontaleX(Plan3D pl, int ax) {
		Area ret = new Area();
		...
		for (int ypos = 0; ypos < this.ySize; ypos ++) {
			...
			Vecteur A = this.getPoint(this.xSize-1, ypos);
			for (int xpos = 0; xpos < this.xSize; xpos ++) {
				Vecteur B = this.getPoint(xpos, ypos);
				Vecteur v = pl.intersection(A, B);
				if (v!= null) { // il y a une intersection avec le plan
					if (!v.equals(A)) {
						if (last == null) {  // première intersection
							last = v;
						} else { // seconde intersection
							ok = true;
							// gérer la position par rapport au plan 
							if (last.getDec(ax).compareTo(v.getDec(ax)) >= 0) {
	//						if (last.getDecY().compareTo(v.getDecY()) >= 0) {
	//						if (pl.donneCote(last) > 0) {
